...
def read_and_process_to_trigram_vecs(data_files, data_path='../data/raw/', sample_size=100, test_split=0.0,
                                     squeeze=True, extract_epitopes=False):
...
def read_dataset(dataset, path, limit=0, concat=False, all_years_label=False):
    """
  ...
  """
    ...
    if all_years_label:
        trigram_idx = np.array(parsed_trigram_idxs)[:, :, 1]
        pre_labels = (trigram_idx[:, 1:] != trigram_idx[:, :-1]).astype(int)
        labels = np.concatenate((pre_labels, np.expand_dims(labels, axis=1)), axis=1)
        print(labels.shape)
